...
export default function useCameraScannerPermissions(isScanning: boolean): boolean {
    ...
    const permission = !!(isScanning
        ? scanningPermission && cameraPermission?.granted
        : cameraPermission?.granted);

    return useMemo(() => permission, [permission]);
}

const CAMERA_PERMISSION_CONFIG: PermissionHookOptions<object> = {
